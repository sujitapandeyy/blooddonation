...
<div id="appointmentModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex justify-center items-center">
    <div class="bg-white p-6 rounded-lg shadow-lg">
        ...
        <div class="mb-4">
            <label for="appointmentHours" class="block text-sm font-medium text-gray-700">Hours</label>
            <select id="appointmentHours" class="w-full p-2 border border-gray-300 rounded">
        ...
        <div class="mb-4">
            <label for="appointmentMinutes" class="block text-sm font-medium text-gray-700">Minutes</label>
            <select id="appointmentMinutes" class="w-full p-2 border border-gray-300 rounded">
  ...
  $(document).ready(function() {
    ...
    $('#saveAppointment').click(function() {
        const hours = $('#appointmentHours').val();
        const minutes = $('#appointmentMinutes').val();

        if (hours !== '' && minutes !== '') {
            const appointmentTime = `${hours.padStart(2, '0')}:${minutes.padStart(2, '0')}`;
    ...
    function updateStatus(requestId, status, appointmentTime = null) {
        console.log({ requestId, status, appointmentTime }); // Add this line
        $.ajax({
            url: 'updatebloodreq.php',
            method: 'POST',
            data: {
