...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user_id) {
        ...
        $donor_id = null; // No user logged in, handle accordingly
    }

    // Insert query with donor_id if applicable
    $insert_sql = "INSERT INTO blood_requests (bloodbank_id, bloodgroup, quantity, requester_name, requester_email, requester_phone, message" . ($donor_id ? ", donor_id" : "") . ") VALUES (?, ?, ?, ?, ?, ?, ?" . ($donor_id ? ", ?" : "") . ")";
    ...
    if ($donor_id) {
        $insert_stmt->bind_param('isissssi', $bloodBankId, $bloodgroup, $quantity, $requester_name, $requester_email, $requester_phone, $message, $donor_id);
    } else {
        $insert_stmt->bind_param('isissss', $bloodBankId, $bloodgroup, $quantity, $requester_name, $requester_email, $requester_phone, $message);
    }
    ...
    <section class="pt-24 pb-16">
        ...
        <div class="">
            <div class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50">
                <!-- <h1 class="text-4xl font-extrabold text-white">Blood Collection of <?= htmlspecialchars($bloodBank['fullname'], ENT_QUOTES, 'UTF-8'); ?></h1> -->
