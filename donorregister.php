...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register'])) {
    ...
    if (empty($fullname) || empty($email) || empty($password) || empty($confirm_password) || empty($phone) || empty($address) || empty($dob) || empty($gender) || empty($blood_group) || empty($height) || empty($weight) || empty($user_type)) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $stmt = $con->prepare("INSERT INTO donor (id, donor_blood_type, dob, weight, gender, height, last_donation_date, availability, days_until_eligible) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("issssssii", $user_id, $blood_group, $dob, $weight, $gender, $height, $last_donation_date_str, $availability, $days_until_eligible);
